...
def update_plot():
    ...
    for renderer in list(renderers.values()):
        if renderer in fig.renderers:
            fig.renderers.remove(renderer)
    renderers.clear()
    for renderer in list(avg_renderers.values()):
        if renderer in fig.renderers:
            fig.renderers.remove(renderer)
    avg_renderers.clear()
    
    if not selected_celltypes:
        fig.title.text = "No cell types selected"
        return
    ...
    if show_greyed and show_individual:
        # First filter complete_df by capacity to match selected capacity
        capacity_filtered_df = complete_df[complete_df[cap] == float(selected_capacity)]
        # Then get the points that aren't in the filtered dataset
